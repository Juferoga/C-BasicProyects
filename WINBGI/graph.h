// Se puede usar PlaySound e incluir sonidos wn wav

/*
La sintaxis inicial es PlaySound(TEXT( "Sonido.wav" ) ,NULL,//Parametros//); 
OJO: Para poder compilar, los sonidos deben estar en la misma carpeta donde esta nuestro código fuente(.CPP). 
Revisaremos tres parámetros que se pueden combinar. con barra vertical
1.SND_LOOP: Hace que el sonido se repita indefinidamente. 
2.SND_SYNC: Pausa nuestro programa hasta que termine de reproducirse el sonido. 
3.SND_ASYNC: No pausa nada. 
*/
...
enum font_types {
    DEFAULT_FONT, TRIPLEX_FONT, SMALL_FONT, SANSSERIF_FONT, GOTHIC_FONT, SCRIPT_FONT, SIMPLEX_FONT, TRISCRIPT_FONT, COMPLEX_FONT, EUROPEAN_FONT, BOLD_FONT
};
...
void drawImage PROTO((char *, int , int ));
void drawImageLimit PROTO(( char *, int , int , int , int));
